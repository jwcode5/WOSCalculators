...
content = content.replace(
    /<!-- Skills -->[\s\S]*?<\/div>\s*<\/div>\s*`;/g,
    '<\/div>\n      <\/div>\n    `;'
);

// 2. Remove Skills dropdown population
content = content.replace(
    /\/\/ Skills\s*\[1, 2, 3, 4\]\.forEach\(skill => \{[\s\S]*?\}\);/g,
    ''
);

// 3. Remove skill change listeners
content = content.replace(
    /document\.querySelectorAll\("\.skill-current-select"\)\.forEach\(sel => \{[\s\S]*?\}\);\s*document\.querySelectorAll\("\.skill-target-select"\)\.forEach\(sel => \{[\s\S]*?\}\);\s*/g,
    ''
);
...
const oldSave = `function saveExpertsState() {
...
const newSave = `function saveExpertsState() {
  const account = getActiveAccount();
  if (!account) return;
...
content = content.replace(/let totalSkillExpNeeded = 0;\s*let totalSkillBooksNeeded = 0;\s*/g, '');
content = content.replace(/let skillExp = 0;\s*let skillBooks = 0;\s*/g, '');
content = content.replace(/\/\/ Skills\s*\[1, 2, 3, 4\]\.forEach\(s => \{[\s\S]*?\}\);\s*/g, '');
content = content.replace(/totalSkillExpNeeded \+= skillExp;\s*totalSkillBooksNeeded \+= skillBooks;\s*/g, '');
